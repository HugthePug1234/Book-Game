...
"""
    ...
    def preview_book_structure(self):
        ...
        structure = f"""
Book Structure Preview for '{self.title}'<br><br>
- Genre: {self.genre}<br>
...
- Conflict Intensity: {self.conflict_intensity}<br><br>
Chapter Outline:<br>
"""
        for i in range(1, self.chapter_count + 1):
            arc_index = min((i - 1) // arc_length, len(narrative_arc) - 1)
            arc_stage = narrative_arc[arc_index]
            structure += f"- Chapter {i} ({arc_stage}): ~{pages_per_chapter} pages, advances the {arc_stage.lower()} arc.<br>"
...
def set_details():
    if request.method == "POST":
        try:
            ...
            generator = BookGenerator()
            generator.__dict__.update(session["generator"])
            output = generator.set_book_details(genre, setting, tone, themes, page_count, chapter_count, conflict_intensity, title fights')
            session["generator"] = generator.__dict__
            flash(output, "success")
            return redirect(url_for("index"))
...
def preview_structure():
    try:
        ...
        generator = BookGenerator()
        generator.__dict__.update(session["generator"])
        output = generator.preview_book_structure()
        session["generator"] = generator.__dict__
        flash(output, "success")
    except Exception as e:
        flash(f"Error: {e}", "error")
